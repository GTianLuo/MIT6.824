...
func (rf *Raft) convertTo(s NodeState) {
	...
	raft.DPrintf("Term %d: server %d convert from %v to %v\n",
...
func (rf *Raft) setCommitIndex(commitIndex int) {
	...
	if rf.commitIndex > rf.lastApplied {
		raft.DPrintf("%v apply from index %d to %d", rf, rf.lastApplied+1, rf.commitIndex)
...
func (rf *Raft) readPersist(data []byte) {
	...
	if d.Decode(&currentTerm) != nil ||
		...
		d.Decode(&logs) != nil {
		raft.DPrintf("%v fails to recover from persist", rf)
		return
	}
				...
				if reply.Success {
					...
					if reply.Term > rf.currentTerm {
						...
						if reply.ConflictTerm != -1 {
							raft.DPrintf("%v conflict with server %d, prevLogIndex %d, log length = %d",
	...
	for i := range rf.peers {
		...
		go func(server int) {
			...
			if rf.sendRequestVote(server, &args, &reply) {
				rf.mu.Lock()
				raft.DPrintf("%v got RequestVote response from node %d, VoteGranted=%v, Term=%d",
...
func (rf *Raft) syncSnapshotWith(server int) {
	...
	raft.DPrintf("%v sync snapshot with server %d for index %d, last snapshotted = %d",
